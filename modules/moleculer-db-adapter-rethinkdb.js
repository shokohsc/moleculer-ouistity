...
class RethinkDBAdapter {
  ...
  /**
     ...
     */
  connect () {
    const { database, table } = this.service.schema
              ...
              r.dbList().run(conn, function (err, res) {
                ...
                if (res.indexOf(database) < 0) {
                  r.dbCreate(database).run(conn, function (err, res) {
                    if (err && err.toString().search(/Database `ouistity` already exists/) !== -1) {
                      resolve(err)
                    }
                    if (err) reject(err)
                    resolve(res)
  ...
  createCursor (params, isCounting = false) {
    if (params) {
      let q
      if (isCounting) { q = r.table(this.table).filter(params.query || {}).count() } else { q = r.table(this.table).filter(params.query || {}) }
      ...
      // Offset
      if (_.isNumber(params.offset) && params.offset > 0) { q = q.skip(params.offset) }

      // Limit
      if (_.isNumber(params.limit) && params.limit > 0) { q = q.limit(params.limit) }

      return q
    }

    // If not params
    if (isCounting) { return r.table(this.table).count() } else { return r.table(this.table) }
  }
